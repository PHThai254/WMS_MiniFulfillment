...
## 10.4. QUY TẮC VIẾT CODE BẮT BUỘC (SOLID & CLEAN ARCHITECTURE)
...
## 11. ĐẶC TẢ THỰC THỂ DỮ LIỆU (CORE ENTITIES & ERD)
...
- **User:** Quản lý nhân sự. Cột: `Id`, `Username`, `PasswordHash`, `Role` (Enum), `WarehouseId` (Nullable - Admin thì null, Staff/QA thì gắn chết vào 1 kho), `RefreshToken` (Lưu chuỗi token), `RefreshTokenExpiryTime` (Lưu ngày hết hạn).
- **Warehouse:** Kho bãi. Cột: `Id`, `Name`, `Address`.
- **Zone:** Khu vực lưu trữ phẳng. Cột: `Id`, `WarehouseId`, `Name` (VD: Dãy A, Khu B).
- **Product:** Danh mục hàng tĩnh. Cột: `Id`, `SKU`, `Barcode`, `Name`, `Category`.
- **Inventory:** Tồn kho thực tế. Cột: `Id`, `WarehouseId`, `ZoneId`, `ProductId`, `Quantity`, `LastRestockedDate` (Dùng để tính FIFO).
- **Receipt (Phiếu Nhập):** Cột: `Id`, `WarehouseId`, `CreatedBy`, `Status` (Enum: Draft, QC_Checked, Completed).
- **ReceiptDetail:** Cột: `Id`, `ReceiptId`, `ProductId`, `ExpectedQuantity`, `ActualQuantity`, `ZoneId` (Nơi cất).
- **Issue (Phiếu Xuất):** Cột: `Id`, `WarehouseId`, `CreatedBy`, `Status` (Enum: Pending, Picking, Handover).
- **IssueDetail:** Cột: `Id`, `IssueId`, `ProductId`, `QuantityToPick`, `PickedQuantity`, `ZoneId` (Nơi nhặt).
